...
- 股票成交量 / 週轉率 / 買進評分，共用同一個 `480` 檔股票顯示池
- ETF 視圖，獨立走 `/api/etf`
...
- Display pool: frontend 股票頁永遠請求 `/api/stocks/top100?mode=volume&limit=480`
- Turnover / buy-score ordering: 都在前端對同一批 480 檔資料重排，不切換股票 API query
...
3. Frontend 股票主畫面固定用 `mode=volume&limit=480`
...
| `BUY_SCORE_QUOTA_MAX_CYCLES` | Max quota-resume cycles |
